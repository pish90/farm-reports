...
- Duplicate detection (all employee-creation paths — bulk import and the single-employee `POST`/`PUT`): an employee is identified by `(farmId, firstName, lastName)` case/whitespace-insensitively, since `employeeId`/`lsNumber` are only generated at insert time and can't be used to spot duplicates beforehand.
  - Bulk import (`EmployeeService.runImport`, shared by CSV/XLSX): a row matching an employee already in the DB, or matching an earlier row in the same file (`dedupeKey`), is a row-level validation error (`"already exists"` / `"Duplicate row in file"`) — nothing is inserted, consistent with the validate-everything-first-then-insert-nothing-on-error contract.
  - Single-employee `createEmployee`/`updateEmployee`: `EmployeeRepository.existsDuplicate` (case/whitespace-insensitive DB query, `excludeId` skips the record being updated) throws `409 Conflict` before any write.
